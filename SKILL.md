...
description: |
  ...
  Requires: curl, jq. Writes: ~/.proxmox-credentials (API token, mode 600).
...
Create API token in Proxmox: Datacenter → Permissions → API Tokens → Add (uncheck Privilege Separation for full access).
...
- **Credential file** must be mode 600 (`chmod 600 ~/.proxmox-credentials`)
- **TLS verification disabled** (`-k`) — Proxmox uses self-signed certs by default
