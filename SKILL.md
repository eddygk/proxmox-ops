...
description: |
  ...
  Requires: curl, jq.
  Credentials: User creates ~/.proxmox-credentials manually (API token, mode 600). This skill reads but never creates or modifies credential files.
  Network: connects to user-configured Proxmox host only (HTTPS). TLS verification is disabled (-k) because Proxmox VE ships with self-signed certificates by default — this is standard practice per Proxmox documentation.
...
- **Credential file** (`~/.proxmox-credentials`) is user-created, not auto-generated by this skill. Must be mode 600 (`chmod 600 ~/.proxmox-credentials`)
- **TLS verification disabled** (`-k` / `--insecure`) — Proxmox VE uses self-signed certificates by default. This is standard per [Proxmox docs](https://pve.proxmox.com/wiki/Certificate_Management). If you deploy a trusted CA cert on your Proxmox node, remove the `-k` flag
- **Least-privilege tokens** — create tokens with only the roles your workflow needs. Full-access tokens are not required for most operations
